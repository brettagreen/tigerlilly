...
class Comment {
	...
	static async create({ userId, text, articleId, postDate }) {
        ...
        const cols = postDate ? '(c.user_id, c.text, c.article_id, c.post_date)' : '(c.user_id, c.text, c.article_id)';
		...
		const result = await db.query(
				`INSERT INTO comments c
                        ${cols}
                        VALUES ${args}
                        RETURNING c.id, c.user_id AS "userId", (SELECT username FROM users WHERE id = c.user_id),
                        c.text, c.post_date AS "postDate", (SELECT article_title AS "articleTitle" from articles where id = c.article_id)`,
			    vals
		);

		const comment = result.rows[0];

		if (!comment) throw new NotFoundError(`No comment by that id: ${id}`);
    ...
    */
	...
	static async edit(id, body) {
		...
		const result = await db.query(`UPDATE comments c
                                    SET 
                                    user_id = $1,
                                    text = $2,
                                    article_id = $3,
    ...
    static async delete(id) {
        const result = await db.query(
                ...
                FROM comments c
                WHERE c.id = $1
                RETURNING c.id, (SELECT username FROM users WHERE id = c.user_id), c.text,
                        (SELECT article_title AS "articleTitle" FROM articles WHERE id = c.article_id)`,
            [Number(id)]
        );
